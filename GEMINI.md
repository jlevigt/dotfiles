...
- **Package Management:** APT (system) and `mise` (language runtimes).
...
- `scripts/`: Modular Bash scripts for specific installation tasks:
    - `utils.sh`: Common logging and utility functions.
...
3. **Declarative Bias:** Prefer updating package lists in `scripts/install-apt.sh` or `mise.toml` over adding manual `apt install` or `curl | sh` commands to scripts.
